class BasicHeader extends HTMLElement {
  ...
  init() {
    new StoreTheme.delayUntilInteraction(this.setHeight.bind(this));
...
class DetailsDropdown extends HTMLDetailsElement {
  constructor() {
    ...
    if (!StoreTheme.config.isTouch && this.trigger === 'hover') {
  ...
  handleSummaryClick(event) {
    ...
    if (!StoreTheme.config.isTouch && this.trigger === 'hover' && this.summaryElement.hasAttribute('data-link')) {
  ...
  async showWithTransition() {
    StoreTheme.Motion.animate(
      ...
      {
        duration: StoreTheme.config.motionReduced ? 0 : 0.6,
        easing: [0.7, 0, 0.2, 1],
        delay: StoreTheme.config.motionReduced ? 0 : 0.1,
      }
    );
    const translateY = this.level === 'top' ? '-105%' : '2rem';
    return StoreTheme.Motion.animate(
  ...
  async hideWithTransition() {
    StoreTheme.Motion.animate(
      ...
      {
        duration: StoreTheme.config.motionReduced ? 0 : 0.3,
        easing: [0.7, 0, 0.2, 1],
      }
    );
    const translateY = this.level === 'top' ? '-105%' : '2rem';
...
class DetailsMega extends DetailsDropdown {
  ...
  async showWithTransition() {
    ...
    return StoreTheme.Motion.animate(
      ...
      {
        duration: StoreTheme.config.motionReduced ? 0 : 0.5,
...
class MenuDrawer extends DrawerComponent {
  ...
  //       delay: StoreTheme.Motion.stagger(0.1),
  //     }
  //   ).finished.then(() => {
...
class MenuProductList extends HTMLElement {
  ...
  initSlide() {
    const slider = new StoreTheme.Carousel(this.container, {
      spaceBetween: 10,
